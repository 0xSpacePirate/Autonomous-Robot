...
public class RLBalance : MonoBehaviour
{
    ...
    private double Gamma = 0.5, LearningRate = 0.5;
    ...
    private int GetRandNextState(int state)
    {
        ...
        List<int> possibleNextStates = GetPossibleNextStates(state);
    ...
    private void Train()
    {
        ...
        for (int epoch = 0; epoch < this.MaxEpochs; ++epoch) // TODO CLEAN UP
        {
            ...
            while (true)
            {
                int nextState = GetRandNextState(currentState);
                List<int> PossibleNextNextStates = GetPossibleNextStates(nextState);
